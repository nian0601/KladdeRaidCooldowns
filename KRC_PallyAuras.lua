...
function KRC_PallyAuras:ToggleVisibility(aValue)
	...
	if aValue == true then
		self.DragableFrame.myBackground:Show()
	else
		self.DragableFrame.myBackground:Hide()
	end
...
function KRC_PallyAuras:CreateDragableFrame()
	...
	self.DragableFrame:SetFrameStrata("BACKGROUND")
	self.DragableFrame:SetFrameLevel(1)
	...
	self.DragableFrame.myBackground = self.DragableFrame:CreateTexture(nil, "LOW")
	self.DragableFrame.myBackground:SetPoint("TOPLEFT", self.DragableFrame, "TOPLEFT", 0, 0)
	self.DragableFrame.myBackground:SetTexture(0.1, 0.07, 0.08, 0.8)
	self.DragableFrame.myBackground:SetWidth(self.DragableFrame:GetWidth())
	self.DragableFrame.myBackground:SetHeight(self.DragableFrame:GetHeight())
end
...
function KRC_PallyAuras:Init()
	...
	if KRC_Core.db.profile.pally_auras.isEnabled ~= nil then
		shouldBeShown = KRC_Core.db.profile.pally_auras.isEnabled
	end

	self:ToggleVisibility(shouldBeShown)
end
	...
	if(KRC_Core.db.profile.pally_auras.isEnabled == true) then
		...
		for i = 1, 7 do
			if(self.myFrames[i]:IsVisible()) then
				self.myFrames[i]:SetPoint("TOPLEFT", self.DragableFrame, "TOPLEFT", 0, -self.myBarHeigth * (count-1))
				count = count + 1
			end
		end

		self.myTotalHeight = KRC_Core.db.profile.pally_auras.height * (count-1)
		self.DragableFrame:SetHeight(self.myTotalHeight)
		self.DragableFrame.myBackground:SetHeight(self.DragableFrame:GetHeight())
	end
end